...
class AnomalyNet(object): 
	...
	def __init__(self,feature_index,FM_weight_dim,feature_id_num,batch_size,
		...
		lstm_layer_num,lstm_hidden_size,is_training,gan_hidden_dim,alpha,beta):
	...
	def FM(self):
		...
		with tf.variable_scope("FM"): 
			for i in range(len(self.feature_id_num)):
				first_order_weight = tf.get_variable('weight_1_embeddings_feature_{}'.format(i),initializer= tf.truncated_normal([self.feature_id_num[i],self.FM_weight_dim]), dtype=tf.float32) 
				second_order_weight = tf.get_variable('weight_2_embeddings_feature_{}'.format(i),initializer= tf.truncated_normal([self.feature_id_num[i],self.FM_weight_dim]), dtype=tf.float32) 
	...
	def Autoencoder(self):
		with tf.variable_scope("autoencoder"):
			self.w_enc = tf.get_variable('weight_encoder',initializer= tf.truncated_normal([2*self.FM_weight_dim,self.autoencoder_hidden_dim]), dtype=tf.float32) 
			self.b_enc = tf.get_variable('bias_encoder',initializer= tf.truncated_normal([1,self.autoencoder_hidden_dim]), dtype=tf.float32) 

			self.w_dec = tf.get_variable('weight_decoder',initializer= tf.truncated_normal([self.autoencoder_hidden_dim,2*self.FM_weight_dim]), dtype=tf.float32) 
			self.b_dec = tf.get_variable('bias_decoder',initializer= tf.truncated_normal([1,2*self.FM_weight_dim]), dtype=tf.float32) 
	...
	def decoder(self):
		...
		instance_vectors_fake = tf.nn.tanh(instance_vectors_fake)
	...
	def LSTM(self):
		...
		state_real = self.initial_state
		state_fake = self.initial_state


		with tf.variable_scope("LSTM"): 
			for time_step in range(self.block_size):
				if time_step > 0: tf.get_variable_scope().reuse_variables()
				cell_output_real,state_real = cell(self.instance_vectors_real[:,time_step,:],state_real) # the last step output is regarded as the block vector
				cell_output_fake,state_fake = cell(self.instance_vectors_real[:,time_step,:],state_fake) # the last step output is regarded as the block vector
				
				
		return cell_output_real,cell_output_fake
	def GAN_discriminator(self):
		with tf.variable_scope("instance"): 
			self.w_instance_hidden = tf.get_variable('w1',initializer= tf.truncated_normal([2*self.FM_weight_dim,self.gan_hidden_dim]), dtype=tf.float32)  
