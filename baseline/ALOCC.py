...
def parse_args():
	...
	parser.add_argument('--block-size', type=int, default=50, help='block size')
	...
	parser.add_argument('--gamma', type=float, default=1.0, help='gamma for loss')
...
class ALOCC(object): 
	...
	def __init__(self,instance_dim,hidden_dim,alpha,beta,gamma,learning_rate):
		...
		with tf.variable_scope("generator_autoencoder"):
			...
			self.w_dec = tf.get_variable('weight_decoder',initializer= tf.truncated_normal([self.hidden_dim,self.instance_dim]), dtype=tf.float32) 
			self.b_dec = tf.get_variable('bias_decoder',initializer= tf.truncated_normal([self.instance_dim]), dtype=tf.float32) 
			
			 
			self.hidden = tf.nn.leaky_relu(tf.matmul(self.data,self.w_enc)+self.b_enc)

			#add random noise here 
			self.hidden = self.hidden + tf.truncated_normal(tf.shape(self.hidden),mean=0.0,stddev=1.0,dtype=tf.float32,seed=1)
			self.fake_data = tf.nn.leaky_relu(tf.matmul(self.hidden,self.w_dec)+self.b_dec)
			 
		#here is the discriminator	

		with tf.variable_scope("discriminator"):
			self.w_d = tf.get_variable('w',initializer= tf.truncated_normal([self.instance_dim,1]), dtype=tf.float32) 
			self.b_d = tf.get_variable('b',initializer= tf.truncated_normal([1]), dtype=tf.float32) 

			self.label_true = tf.matmul(self.data,self.w_d)+self.b_d
			self.label_fake = tf.matmul(self.fake_data,self.w_d)+self.b_d

		loss_g1 = tf.reduce_mean(tf.nn.sigmoid_cross_entropy_with_logits(logits=self.label_fake, labels=tf.ones_like(self.label_fake)),axis = 1)
		loss_g2 = tf.reduce_mean(tf.nn.sigmoid_cross_entropy_with_logits(logits=self.fake_data, labels=tf.nn.sigmoid(self.data)),axis = 1)
		self.generator_loss = self.alpha* loss_g1 + self.beta*loss_g2

		loss_d1 = tf.reduce_mean(tf.nn.sigmoid_cross_entropy_with_logits(logits=self.label_fake, labels=tf.zeros_like(self.label_fake)),axis = 1)
		loss_d2 = tf.reduce_mean(tf.nn.sigmoid_cross_entropy_with_logits(logits=self.label_true, labels=tf.ones_like(self.label_true)),axis = 1)
		self.discriminator_loss = self.gamma*(loss_d1+loss_d2)
		
		self.total_loss = self.generator_loss + self.discriminator_loss  # total loss is here 	
		self.average_loss = tf.reduce_mean(self.total_loss) # average a batch of instances' loss
		
		self.D_train = tf.train.RMSPropOptimizer(self.learning_rate).minimize(tf.reduce_mean(self.discriminator_loss))
		self.G_train = tf.train.RMSPropOptimizer(self.learning_rate).minimize(tf.reduce_mean(self.generator_loss))
