...
blog详解: http://www.twistedwg.com/2018/09/13/GANomaly.html 
这样用encoder-decoder 思想，效率会大大提升，不需要每次来一个数据都要单独优化一遍GAN了 
PPT: http://mlg.postech.ac.kr/~readinglist/slides/20180911.pdf 
...
class GANomaly(object): 
	...
	def __init__(self,instance_dim,hidden_dim,alpha,beta,gamma,learning_rate):
		...
		with tf.variable_scope("generator_autoencoder"):
			...
			self.w_z = tf.get_variable('w_z',initializer= tf.ones([self.instance_dim,self.hidden_dim]), dtype=tf.float32) 
			self.b_z = tf.get_variable('b_z',initializer= tf.ones([self.hidden_dim]), dtype=tf.float32) 
			...
			self.z = tf.nn.leaky_relu(tf.matmul(self.fake_data,self.w_z)+self.b_z) # decoder for the 
		...
		loss_g1 = tf.reduce_mean(tf.nn.sigmoid_cross_entropy_with_logits(logits=self.fake_data, labels=tf.nn.sigmoid(self.data)),axis = 1)
		loss_g2 = tf.reduce_mean(tf.nn.sigmoid_cross_entropy_with_logits(logits=self.z, labels=tf.nn.sigmoid(self.hidden)),axis = 1)
